...
import { useReportStore, useReportFilters, useDistributorReport } from '@/store/report.store'
import { getDistributorPerformanceReportAction } from '../actions/report.actions'
...
function DistributorTable() {
  const data = useDistributorReport()
  const rows = data?.rows ?? []

  if (!rows.length) {
    return <p className="py-8 text-center text-sm text-slate-400">No distributor leads for this period</p>
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 dark:border-slate-800">
            {['Distributor', 'Leads Received', 'Leads Converted', 'Conversion Rate'].map((h) => (
...
export default function DistributorPerformanceReport() {
  const { loading, setDistributor, setLoading, setFilters } = useReportStore()
  const filters   = useReportFilters()
  const data      = useDistributorReport()
  const isLoading = loading['distributor'] === 'loading' || loading['distributor'] === 'idle'
  const error     = useReportStore((s) => s.error['distributor'])
  ...
  function runReport() {
    setLoading('distributor', 'loading')
    getDistributorPerformanceReportAction(filters).then((r) => {
      if (r.ok) { setDistributor(r.data); setLoading('distributor', 'ready') }
      else       setLoading('distributor', 'error', r.error)
    })
  }
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="flex flex-col gap-1 min-w-[160px]">
          <label className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Distributor</label>
          <select
            value={filters.distributorId ?? ''}
            onChange={(e) => setFilters({ distributorId: e.target.value || undefined })}
      ...
      {data && (
        <>
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm p-5">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-4">Distributor Breakdown</h3>
            <DistributorTable />
          </div>
        </>
      )}
