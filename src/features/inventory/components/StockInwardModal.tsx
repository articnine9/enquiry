...
export default function StockInwardModal({
  ...
  function handleItemChange(index: number, field: keyof InwardItemRow, value: unknown) {
    const updated = [...items]
    ...
    if (field === 'productId') {
      ...
      current.unitPrice = prod?.costPrice || 0
    } else {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Inward receipt ${res.data.transactionNo} recorded successfully`)
        onSuccess()
        onClose()
      } else {
        setError(res.error || 'Failed to record inward receipt')
      }
    } catch (err) {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-4xl overflow-hidden my-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Receive inventory from suppliers, production, or factory purchase
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold uppercase text-slate-600 dark:text-slate-300 mb-1.5">
                Reference / Invoice / PO No.
              </label>
              <input
                type="text"
                placeholder="e.g. PO-2026-9481 or INV-8821"
                value={referenceNo}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/50 hover:bg-emerald-100 transition-colors"
              >
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 text-slate-500 uppercase">
                    <tr>
                      <th className="px-3 py-2.5 min-w-[200px]">Product *</th>
                      <th className="px-3 py-2.5 min-w-[120px]">Batch No.</th>
                      <th className="px-3 py-2.5 min-w-[130px]">Expiry Date</th>
                      <th className="px-3 py-2.5 min-w-[90px]">Qty *</th>
                      <th className="px-3 py-2.5 min-w-[100px]">Unit Cost (₹)</th>
                        ...
                        <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                          ...
                          <td className="px-3 py-2">
                            <select
                              ...
                              className="w-full px-2 py-1.5 rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                            >
                              {products.map(p => (
                          ...
                          <td className="px-3 py-2">
                            <input
                              ...
                              className="w-full px-2 py-1.5 rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none uppercase font-mono"
                            />
                          </td>
                          ...
                          <td className="px-3 py-2">
                            <input
                              ...
                              className="w-full px-2 py-1.5 rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-semibold focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                            />
                          </td>
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-slate-50 dark:bg-slate-800/60 border-t border-slate-200 dark:border-slate-700">
                <span className="text-xs font-medium text-slate-500">Total Inward Valuation:</span>
          ...
          <div>
            ...
            <textarea
              rows={2}
              placeholder="Delivery condition, vehicle number, inspection details..."
              value={notes}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              ...
              className="px-5 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 rounded-lg shadow-sm disabled:opacity-50 transition-colors flex items-center gap-2"
            >
              {isSubmitting ? <span>Recording...</span> : <span>Confirm Goods Receipt</span>}
            </button>
          </div>
        </form>
      </div>
